...
if ( (isset($_GET['id'])) && (is_numeric($_GET['id'])) ) { 
		 ...
		 $edit_diploms=Diplome_demmande::trouve_par_sql("select * from diplome_demmande where poste='".$edit->Nom_poste."'");
	...
	// new object document
		$diplome_demmande = new Diplome_demmande();
 $SQL = $bd->requete("SELECT * FROM `diplome_demmande`") ;
																	$nbr_doc = 0;
																	while ($row=mysqli_fetch_array($SQL)){	
		$nbr_doc=$row['id']+1;
			}
$diplome_demmande->id =$nbr_doc;
	$diplome_demmande->diplome = htmlentities(trim($_POST['nom_diplome_demmande']));
	
	$diplome_demmande->poste = htmlentities(trim($_POST['poste']));
	
	if (empty($errors)){
   		if ($diplome_demmande->existe()) {
			$msg_error = '<p style= "font-size: 20px; ">  !!??  يـوجد من قبل " ' . $diplome_demmande->diplome . ' " </p><br />';
			
		}else{
			$diplome_demmande->save();
 		$msg_positif = '<p style= "font-size: 20px; ">   تم تسجيــــل  " ' . $diplome_demmande->diplome . ' "   ... بنجــــاح        </p><br />';
...
<?php
$titre = "تحديث الشهادات";
$active_menu = "index";
$header = array('diplome_demmande');
...
?>
                ...
                <!-- START BREADCRUMB -->
                <ul class="breadcrumb">
					  <li><a href="index.php">الصفحة الرئيسية</a> &nbsp;/ &nbsp;  <a href="list_diplome.php">الشهادات المطلوبة</a>  </li>  
                ...
                <div class="page-title">                    
                    <h2> تحديث الشهادات <span class="fa fa-plus-square"></span> </h2>
                </div>   
                            ...
                            <div class="panel panel-default">
                                <div class="panel-heading">
                                    ...
                                    <ul class="panel-controls">
                                        <li> <h3 class="panel-title"><strong>المعلومات الخاصة بالشهادة المطلوبة</strong></h3></li>
                                    </ul>
                                </div>
...
?>                                                 <?php */?>
                                    ...
                                    <div class="form-group" style ="dir:rtl;" >
                                        <div class="col-md-6 col-xs-12">                                            
                                            <div class="input-group">
                                                ...
                                                <input type="text" name ="nom_diplome_demmande" class="form-control"   style= "text-align:right;" data-live-search="true"  />
                                            </div>                                            
                                        </div>
                                        <label class="col-md-3 col-xs-12 control-label">اسم الشهادة</label>
                                    </div>
                        ...
                        <span>&nbsp;&nbsp;</span>
                                ...
                                <div class="panel-footer">
                                    ...
                                    <p>&nbsp;</p>
                                    <div class="panel-body">
                                      <p style="direction:rtl;font-size:24px"><u><b>الشهادات المطلوبة في منصب  :<?php echo $edit->Nom_poste; ?></b></u></p>
                                      <table class="table table-bordered" style="direction:rtl"  id="customers3">
                                        <tr>
                                          <td>اسم الشهادة</td>
                                          <td>حذف</td>
                                        </tr>
                                        <?php foreach($edit_diploms as $edit_diplom){?>
                                        <tr id="<?php echo $edit_diplom->id;  ?>">
                                          <td><div class="input-group">
                                            <label name="<?php echo $edit_diplom->diplome; ?>" class="form-control"  style= "text-align:right;" value = "<?php echo $edit_diplom->diplome;  ?>" ><?php echo $edit_diplom->diplome; ?></label>
                                          </div></td>
                                          <td><p class="btn btn-danger btn-rounded "  onclick="delete_row('<?php echo $edit_diplom->id;?>');"><span class="glyphicon glyphicon-trash"></span></p></td>
                                        </tr>
                                        <?php }?>
                                      </table>
