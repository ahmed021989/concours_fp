...
class Specialite_demmande {
	
	protected static $nom_table="specialite_demmande";
	protected static $champs = array('id','poste','specialite');
	public $id;
	public $poste;
	public $specialite;
  ...
  public function nom_compler() {
	...
	public  function  existe(){
	 global $bd;
	 $sql  = "SELECT * FROM ".self::$nom_table." ";
    $sql .= "WHERE poste = '".$this->poste."' and specialite= '".$this->specialite."'";
  ...
  public static  function trouve_par_id($poste) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE poste ='{$poste}' ");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  
   public static  function trouve_par_id2($id) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id ={$id} ");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
...
$sql .= " WHERE id =". $bd->escape_value($this->id) ;
$bd->requete($sql);
return($bd->affected_rows() == 5) ? true : false ;
}
