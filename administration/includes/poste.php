...
class Poste {
	...
	protected static $nom_table="poste";
	protected static $champs = array('Id_poste','Nom_poste','nb_poste','excercice','Date_debut', 'Date_fin','Type_recr','date_convocation','date_debut_conv','date_fin_conv');
	public $Id_poste;
	public $Nom_poste;
	public $Date_debut;
	public $nb_poste;
	...
	public $date_debut_conv;
	public $date_fin_conv;
  ...
  public function nom_compler() {
	...
	public  function  existe(){
	 global $bd;
	 $sql  = "SELECT * FROM ".self::$nom_table." ";
    $sql .= "WHERE Nom_poste = '".$this->Nom_poste."' and excercice='".$this->excercice."'";
	...
	public  function  existe_on_condidat($poste,$excercice){
	 global $bd;
	 $sql  = "SELECT * FROM  condidat"." ";
    $sql .= "WHERE grade = '".$poste."' and excercice = '".$excercice."' ";
	//$sql .= "OR email = '".$this->email."' ";
    $sql .= "LIMIT 1";
    $result_array = self::trouve_par_sql($sql);
		return !empty($result_array) ? array_shift($result_array) : false;
  ...
  public static function trouve_tous_excercice($excercice) {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." where excercice='".$excercice."'");
  }
  
  public static function trouve_par_id($id) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE Id_poste ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
   public static function trouve_par_id1($id) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE Nom_poste ='{$id}' LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
 	public static function trouve_concour_titre() {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." where Type_recr = 'concour_sur_titre' and  Nom_poste = ".$this->Nom_poste );
  }
  public static function trouve_concour_exame() {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." where Type_recr = 'concour_sur_exam' and  Id_poste = ".$this->Id_poste );
  }
  
   public static function trouve_par_date($date=0){
	$q =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE Date_fin >= '{$date}'";
    return  self::trouve_par_sql($q);
	}
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->Id_poste)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE Id_poste =". $bd->escape_value($this->Id_poste) ;
$bd->requete($sql);
