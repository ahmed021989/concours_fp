...
class Notes {
	...
	protected static $nom_table="notes";
	protected static $champs = array('id_note','note_deplom','note_exame','note_date_deplom', 'note_relve_note','note_experiance','note_majour','note_ecole_sup','note_forma_comple','note_etud_realise','moyen','code');
	public $id_note;
	public $note_deplom;
	public $note_exame;
	public $note_date_deplom;
	public $note_relve_note;
	public $note_experiance;
	public $note_majour;
	public $note_ecole_sup;
	public $note_forma_comple;
	public $note_etud_realise;
	public $moyen;
	public $code;
    ...
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE code ='{$code}' LIMIT 1 ");
  	  ...
  	  public static function trouve_moyen() {
		return self::trouve_par_sql("SELECT * FROM notes  ORDER BY moyen ASC ");
  }
 ...
 // public static function trouve_par_moyen($id) {
   // $result_array = self::trouve_par_sql("SELECT * FROM notes orderby(moyen)");
	//	return !empty($result_array) ? array_shift($result_array) : false;
  //}
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->id_note)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE id_note =". $bd->escape_value($this->id_note) ;
$bd->requete($sql);
...
$sql .= " WHERE code =". $bd->escape_value($this->code) ;
$bd->requete($sql);
return($bd->affected_rows() == 1) ? true : false ;
}
