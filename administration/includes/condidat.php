...
class Condidat {
	...
	protected static $nom_table="condidat";
	protected static $champs = array('code','grade','nom','prenom','sexe','fils_de','et_fils_de','date_naisance','lieu_naisance','nationalite','marie','nbr_enfants','fils_chahid','andicape','nature_endicape','commune','wilaya','adresse','n_telephone','email','service_national','n_piece_sv','date_deliv_piece_sv');
	public $code;
	public $grade;
	public $nom;
	public $prenom;
	...
	public $fils_de;
	public $et_fils_de;
	public $date_naisance;
	public $lieu_naisance;
	public $nationalite;
	public $marie;
	public $nbr_enfants;
	public $fils_chahid;
	public $andicape;
	public $nature_endicape;
	public $commune;
	public $wilaya;
	public $adresse;
	public $n_telephone;
	public $email;
	public $service_national;
	public $n_piece_sv;
	public $date_deliv_piece_sv;
  ...
  public function nom_compler() {
	...
	public  function  existe(){
	 global $bd;
	 $sql  = "SELECT * FROM ".self::$nom_table." ";
    $sql .= "WHERE Nom_filiere = '".$this->Nom_filiere."' ";
	...
	public static function trouve_tous() {
  	...
  	public static function trouve_tous1() {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." where code=(select * from Dossier_accept where accept='oui')");
  }
  
  
  
  public static  function trouve_par_id($code) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE code ='{$code}' LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
    public static  function trouve_par_id_excercice($code,$excercice) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE code ='{$code}' and excercice ='{$excercice}' LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  
  
  
  public static function trouve_par_poste($code) {
    $result_array = self::trouve_par_sql("SELECT * FROM condidat WHERE grade ={$code} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->Id_filiere)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE Id_filiere =". $bd->escape_value($this->Id_filiere) ;
$bd->requete($sql);
